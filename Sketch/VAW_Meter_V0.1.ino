...
void IRAM_ATTR readEncoderISR(){rotaryEncoder.readEncoder_ISR(); }
  ...
  int x,y;
  char PWM0;
  int PWM,PWM_power,display_backlight,rty;
  uint32_t ms_display,ms_led,ms_PWM,ms_SP,Time;
  bool flag_led,Accumulation_flag,flag_enc_but,flag_enc;
  float Voltage,Current,Power,Temperature,Charge,Energy,Voltege_fil,Curent_fil,Set_voltege,Encoder_memory;
...
//====================================================================================
 ...
 INA.setBusVoltageConversionTime(5);   // Час перетворення  АЦП вольтметра us  50=0,84=1,150=2,280=3,540=4,1052=5,2074=6,4120=7 us 
 INA.setShuntVoltageConversionTime(5); // Час перетворення  АЦП Амперметра  50,84,150,280,540,1052,2074,4120 us
 INA.setTemperatureConversionTime(5);   // Час перетворення  АЦП температури 50,84,150,280,540,1052,2074,4120 us
 INA.setAverage(5); // Кількість вибірок 1=0,4=1,16=2,64=3,128=4,254=5,512=6,1024=7 
...
analogReadResolution(11); // Розщирення АЦП ESP32
 ...
 rotaryEncoder.setBoundaries(-2000, 1000, false); //minValue, maxValue, circleValues true|false (when max go to min and vice versa)
 rotaryEncoder.setAcceleration(150);

 Display_ramka();
 EEPROM.begin(512);
 //Set_voltege = EEPROM.read(0);  // Чтение данных
 EEPROM.get(0, Encoder_memory);// - читает данные по адресу и сам записывает их в данные - указанную переменную
...
//====================================================================================
void loop() {
 if( display_backlight < 255 ){
  rty++;
 display_backlight= pow(rty,3)/50;
  if( display_backlight > 255 ){display_backlight=255;}
  ledcWrite(TFT_BL, display_backlight);
 }
