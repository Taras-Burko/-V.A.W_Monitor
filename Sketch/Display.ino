void Display(){
  ...
  // String  V = "V";
   ...
   if( Current < 0 ){ x = 53;}
   else if ( Current >= 10 ){ x = 43;}
   else { x = 60 ; }
   
   tft.setTextColor(TFT_RED, TFT_BLACK, true);
   tft.setCursor(x,78);         // 
   tft.println(Current_display);
   //tft.drawString(Curent_display, 290, 80);
   ...
   tft.drawString(Power_display, 110, 143);
   tft.unloadFont();
   ...
   tft.setTextColor(TFT_WHITE, TFT_BLACK);
     //tft.drawString(Voltege_display, 120, 40, 6);// Print the test text in the custom font
     //tft.drawString("V",220, 40, 6);// Print the string name of the font
   
   //tft.drawString(Curent_display, 160, 110, 4);// Print the test text in the custom font               
   //tft.drawString("A",240, 110, 4);// Print the string name of the font
   
   //tft.drawString(Power_display, 160, 140, 4);// Print the test text in the custom font
   //tft.drawString("W", 240, 140, 4);// Print the string name of the font
  
   tft.drawString(Charge_display, 45, 174, 2);// Print the test text in the custom font
   //tft.drawString("mA/h", 100, 180, 2);// Print the string name of the font

   tft.drawString(Energy_display, 202,174, 2);// Print the test text in the custom font
   //tft.drawString("Wh", 260, 180, 2);// Print the string name of the font
   
   tft.drawString(Temperature_display, 37,213, 2);// Print the test text in the custom font
   //tft.drawString("°C", 50, 213, 2);// Print the string name of the font

   tft.drawString(PWM_display, 55,213, 2);// Print the test text in the custom font
   //tft.drawString("%", 92, 213, 2);// Print the string name of the font

   tft.drawString(cicle_display, 200,213, 2);// Print the test text in the custom font

    
    if (flag_enc_but ==1 ) {
       tft.drawString(Set_voltege_display, 250 ,213, 2);// Print the test text in the custom font
   tft.drawString("V", 300, 213, 2);// Print the string name of the font
    }
    else {
      
      tft.fillRect(215, 208, 95, 23, TFT_BLACK);//x, y, w, h, color
    }
...
tft.drawWideLine(0, 0, 320,0,2, TFT_WHITE, TFT_BLACK);
    ...
    tft.pushImage(7,203,30, 30, celsius_image);
    tft.pushImage(40,203,30, 30, fan_image);
}